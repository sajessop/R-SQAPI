...
```{r}
## TO DO: add support for POST and PATCH verbs eg (if (verb == "POST") allow sending of json)
export <- function(url, verb, token, poll = TRUE, filename) {
  response <- httr::VERB(
    verb = verb,
    url = url,
    config = httr::add_headers(Authorization = paste("Bearer", token)),
...
```{r}
 r3 <- append_url(api$host,
                   ...
                   my_filters_3,
                   my_params_3) %>% export("GET", api$auth, filename = "test3.csv")
meta <- r3$headers$`x-content-metadata`
write(meta, "meta.json")
