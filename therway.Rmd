...
query_filters <- function(base_url,
  ...
  filter <- list(name = name, op = op)
  if (!is.null(val)) filter$val <- val
  
  # Convert filters to JSON and URL encode
  json_query <- jsonlite::toJSON(list(filters = list(filter)), auto_unbox = TRUE)
  # encoded_query <- utils::URLencode(json_query)
