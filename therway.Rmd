...
request <- function(api,
                    ...
                    query_parameters = NULL,
                    verb) {
  url <- append_url(api = api, endpoint = endpoint, query_filters = query_filters, query_parameters = query_parameters)
  token <- api$auth
  response <- httr::VERB(
    verb = verb,
    url = url,
...
# Example 0 - A simple query
{
  ...
  r <- append_url(api, "api/annotation_set", my_filters) %>% request("GET", api)
  
  rtest <- request(api = api, endpoint = "api/annotation_set", query_filters = my_filters, verb = "GET")
  # parse
  p <- parse(r)
  
  # look at url
  my_url <- append_url(api, "api/annotation_set", my_filters)
...
# Example 1 - A nested query to Get all media_collections that contain media from a specific campaign matching the key "Batemans201011"
  ...
  # Append filters and send request
  r1 <- append_url(api, "api/media_collection", my_filters1) %>% request("GET", api)
  
  # parse 
  p1 <- parse(r1)
  
  # look at url
  my_url1 <- append_url(api, "api/media_collection", my_filters1)
...
# Example 2 - A simple query to Get all annotations that match annotation_set_id = 5432 and specify pagination params
{
  ...
  r2 <- append_url(api, "api/annotation", my_filters_2, my_params_2) %>% request("GET", api)
  # Parse
  p2 <- parse(r2, my_params_2)
  # look at url
  my_url2 <- append_url(api, "api/annotation", my_filters_2, my_params_2)
...
# Example 3 - Get request to export endpoint using status polling and template = data.csv to download the data as a .csv
{
  ...
  r3 <- append_url(api,
                   ...
                   my_filters_3,
                   my_params_3) %>% export("GET", api, metadata_filename = "my_metadata3.json")

  # Parse
  p3 <- parse(r3, my_params_3)

    # Look at url
  my_url_3 <- append_url(api,
                         "api/media_collection/13453/export",
                         my_filters_3,
                         my_params_3)
...
# Example 4 - Get request to export endpoint using status polling and defining limit and offset parameters
{
  ...
  r4 <- append_url(api,
                   "api/media_collection/13453/export",
                   my_filters_4,
                   my_params_4) %>% export("GET", api)

  # Parse
  p4 <- parse(r4, my_params_4)
  
  # Look at url
  my_url_4 <- append_url(api,
                         "api/media_collection/13453/export",
                         my_filters_4,
                         my_params_4)
...
# Example 5 - Get request to export endpoint using status polling and template = data.csv to download the data as a .csv with order_by
{
  ...
  r5 <- append_url(api,
                   "api/media_collection/13453/export",
                   my_filters_5,
                   my_params_5) %>% export("GET", api)

  # Parse
  p5 <- parse(r5, my_params_5)
  
  # Look at url
  my_url_5 <- append_url(api,
                         "api/media_collection/13453/export",
                         my_filters_5,
                         my_params_5)
...
simple_url <- append_url(host = api, endpoint = "api/annotation/5432") %>% request("GET", api, filename = "simple.json")
```
