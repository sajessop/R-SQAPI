...
```{r}
append_filters <- function(host, endpoint, query_filters) {
  
  # Finalise formatting of query filters
...
```{r}
test <- httr::VERB("GET", "https://squidle.org/api/annotation_set?q=%7B%22filters%22%3A%5B%7B%22name%22%3A%22id%22%2C%22op%22%3A%22eq%22%2C%22val%22%3A%225432%22%7D%5D%7D&page=2&results_per_page=1000", config = httr::add_headers(Authorization = paste("Bearer", api$auth)), write_disk("test_content_values.json", overwrite = TRUE))

  json <- jsonlite::fromJSON(
    content(test, 'text', encoding = "UTF-8"),
    simplifyVector = TRUE,
    flatten = TRUE
  )
```
...
json_query <- jsonlite::toJSON(test_json_query, auto_unbox = TRUE)
# Output the final JSON query
print(json_query)
print(test_json_query)

test_url <- append_filters(api$host, "api/media_collection", my_filters)
cat(URLdecode(test_url))
```
