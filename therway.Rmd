...
request <- function(url, verb, token, filename) {
  ...
  # if (response$status_code <= 299 && response$headers$`content-type` == "application/json") {
...
simple_url <- append_url(host = api$host, endpoint = "api/annotation/5432") %>% request("GET", api$auth, filename = "test.json")
#cat(simple_url)}
```
